...
- Size ceilings (spec §8, amended per D8 2026-07-06): CLAUDE.md ≤ 11,400 chars (was 11,000); local.md ≤ 1,500 chars; combined ≤ 12,500 (binding budget, unchanged).
...
### Task 6: Adopt the Session Lifecycle block (spec D8 amendment)

**Files:**
- Modify: `/Users/harper/.claude/CLAUDE.md` (full replacement)

**Interfaces:**
- Consumes: the shipped Task 3 file (r2) as base; spec D8 for rationale.
- Produces: final CLAUDE.md with `# Session lifecycle` section; the string `subagent-driven-development` moves from Designing software into it.

- [ ] **Step 1: Replace the entire file with exactly this content**

The authoritative copy of this block also lives at `/Users/harper/.claude/.superpowers/sdd/task-6-content-draft.md` — you may `cp` it over `/Users/harper/.claude/CLAUDE.md` instead of transcribing, then verify byte-identity with `diff`. Contents are FINAL — do not edit:
...
Run: `wc -c /Users/harper/.claude/CLAUDE.md /Users/harper/.claude/docs/local.md`
Expected: 11383 for CLAUDE.md (≤ 11400); 884 for local.md; total 12267 (≤ 12500)

- [ ] **Step 3: Must-hit greps (each ≥ 1)**

Run each: `grep -c "PATTERN" /Users/harper/.claude/CLAUDE.md` for: `THE SUN IS DYING`, `Doctor Biz`, `ABOUTME`, `I AUTHORIZE YOU TO SKIP WRITING TESTS THIS TIME`, `scripts/check`, `NEVER INVENT`, `Rule #1`, `subagent-driven-development`, `Session lifecycle`
Expected: every count ≥ 1

- [ ] **Step 4: Must-miss grep (zero hits) + include check**

Run: `grep -E "GPT-5\.5|Opus 4\.6|memory MCP|Context Decay|worktress|expliict|refering|--production|0\.7\.4" /Users/harper/.claude/CLAUDE.md; echo $?`
Expected: no output, exit 1
Run: `grep -c "^@" /Users/harper/.claude/CLAUDE.md && tail -1 /Users/harper/.claude/CLAUDE.md`
Expected: 1, and `@~/.claude/docs/local.md`

- [ ] **Step 5: Dedup checks (the moved/superseded lines must not appear twice)**

Run: `grep -c "I highly prefer" /Users/harper/.claude/CLAUDE.md`
Expected: 1 (in Session lifecycle only)
Run: `grep -c "intermediate results to files" /Users/harper/.claude/CLAUDE.md; echo $?`
Expected: 0 hits, exit 1 (clause superseded by lifecycle bullet 1)
